    ...
    public partial class BookStoreContextSeed : DbContextSeed
    {
        private static IEnumerable<User> GetPredefineUsers(string contentRootPath)
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                var user = CreateNewUser(values[0], values[1], values[2], values[3], values[4], values[5]);
                users.Add(user);
            }
            return users;
        }

        private static User CreateNewUser(string id, string name, string fullName, string password, string email, string phoneNumber)
        {
            return new User(new Guid(id), name, fullName, password, email, phoneNumber);
        }
    }
}