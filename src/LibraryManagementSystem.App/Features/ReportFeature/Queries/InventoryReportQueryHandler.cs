...
namespace BookStoreManagementSystem.App.Features.ReportFeature.Queries
{
    public class InventoryReportQueryHandler : IListQueryHandler<InventoryReportQuery, InventoryReportDto>
    {
        ...
        {
            var bss = await _context.BookStoresStorage
                .AsNoTracking()
                .Include(x => x.Book)
            ...
            foreach (var xxx in bss)
            {
                var minDifferentStartDate = inventory.Where(x => x.BookId == xxx.BookId)?.Min(x => x.Date - request.StartDate);
                var minDifferentEndDate = inventory.Where(x => x.BookId == xxx.BookId)?.Min(x => request.EndDate - x.Date);

                var startDate = inventory.FirstOrDefault(x => x.BookId == xxx.BookId && x.Date - request.StartDate == minDifferentStartDate).Date;
                var startDateInventory = inventory.Where(x => x.BookId == xxx.BookId && x.Date >= startDate);
                var startDateNumber = startDateInventory.Sum(x => x.ChangeNumber);

                var endDate = inventory.FirstOrDefault(x => x.BookId == xxx.BookId && request.EndDate - x.Date == minDifferentEndDate).Date;
                var endDateInventory = inventory.FirstOrDefault(x => x.BookId == xxx.BookId && request.EndDate - x.Date == minDifferentEndDate);
                var endDateNumber = inventory.Where(x => x.BookId == xxx.BookId && x.Date >= endDateInventory.Date).Sum(x => x.ChangeNumber);

                var inventoryReport = new InventoryReportDto
                {
                    BookId = xxx.BookId,
                    BookName = xxx.Book.Name,
                    OpeningStocks = xxx.Availability - startDateNumber,
                    EndingStocks = xxx.Availability - endDateNumber
                };
